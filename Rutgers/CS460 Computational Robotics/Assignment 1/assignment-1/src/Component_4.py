...
"""
function interpolate_arm()
"""
def interpolate_arm(start, goal):
    start = np.array(start)
    goal = np.array(goal)
    
    # Defining a reasonable number of interpolation steps based on 
    #   the angular distance.
    delta_theta = np.linalg.norm(goal - start) # Total angular difference.
    steps = int(delta_theta / 0.1) # Example: 0.1 radians per step.
    
    # Generating linearly spaced configurations between start and goal.
    interpolated_path = [start + (goal - start) * t for t in np.linspace(0, 1, steps)]
    
    return interpolated_path

"""
function forward_propagate_arm()
"""
def forward_propagate_arm(start_pose, plan):
    path = [start_pose]
    
    # Looping through (velocity, duration) pair in the plan.
    for velocity, duration in plan:
        # Get the last pose in the path (current configuration of the arm).
        last_pose = np.array(path[-1])
        
        # Calculate the new pose by applying the vector over the duration.
        delta_theta = np.array(velocity) * duration # Change in angle = velocity * time.
        
        # The new pose is the last pose + change in angle (delta_theta).
...
def visualize_arm_path(path):
    ...
    # Function to compute the (x, y) coordinates of the arm's joints
    #   based on angles.
...
def visualize_arm_path(path):
    ...
    # Initialize plot elements: base to joint 1 and joint 1 to joint 2.
    arm_line, = ax.plot([], [], 'o-', lw=4)  # Unpack the list by adding the comma
